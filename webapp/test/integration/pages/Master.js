				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("softwareID") === oObject.softwareID;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].softwareID;
										} else {
											sCurrentId = aItemsNotInTheList[0].softwareID;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("softwareSet", {
											softwareID : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("softwareID"),
							title: oBindingContext.getProperty("SoftwareName")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "softwareSet (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'softwareSet (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("SoftwareName");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all softwareSet to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "softwareSet (" + iExpectedLength + ")"}),
									success : function () {
